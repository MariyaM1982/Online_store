...
class TestProductCategory(unittest.TestCase):
    def setUp(self):
        ...
        self.category = Category("Фрукты", "Разнообразные фрукты", [])

    def test_new_product_creation(self):
        """Тест на создание нового продукта."""
        product1 = Product.new_product(self.product1_info,
    ...
    def test_product_update_existing(self):
        ...
        updated_product = Product.new_product(product_update_info,
                                              self.existing_products)

        self.assertEqual(updated_product.quantity,
                         15)  # Проверка обновленного количества
    ...
    def test_add_product_to_category(self):
        ...
        self.assertIn(product1,
                      self.category._Category__products)  # Проверка добавления продукта
        self.assertEqual(Category.get_product_count(),
                         1)  # Проверка общего количества продуктов

    def test_get_category_products(self):
        """Тест на получение списка продуктов из категории."""
        product1 = Product.new_product(self.product1_info,
                                       self.existing_products)
        product2 = Product.new_product(self.product2_info,
                                       self.existing_products)
        self.category.add_product(product1)
    ...
    def test_no_products_in_category(self):
        ...
        self.assertEqual(self.category.products.strip(),
                         "Нет продуктов в категории.")
